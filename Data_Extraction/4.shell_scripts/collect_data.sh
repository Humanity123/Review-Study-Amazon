...
if [ $DOMAIN != "INDIA" ] && [ $DOMAIN != "USA" ] && [ $DOMAIN != "UK" ]; then
	echo "No Such Domain"
	exit 1
fi
...
if [ $DOMAIN != "INDIA" ]; then
	python LINK_INDIA_SCRIPT $DATABASE_DIREC >> LOG_FILE_INDIA
	python METADATA_INDIA_SCRIPT $DATABASE_DIREC >> LOG_FILE_INDIA &
	python REVIEW_INDIA_SCRIPT $DATABASE_DIREC >> LOG_FILE_INDIA &
	wait

elif [ $DOMAIN != "USA" ]; then
	python LINK_USA_SCRIPT $DATABASE_DIREC >> LOG_FILE_USA
	python METADATA_USA_SCRIPT $DATABASE_DIREC >> LOG_FILE_USA &
	python REVIEW_USA_SCRIPT $DATABASE_DIREC >> LOG_FILE_USA &
	wait

else
	python LINK_UK_SCRIPT $DATABASE_DIREC >> LOG_FILE_UK
	python METADATA_UK_SCRIPT $DATABASE_DIREC >> LOG_FILE_UK &
	python REVIEW_UK_SCRIPT $DATABASE_DIREC >> LOG_FILE_UK &
	wait
